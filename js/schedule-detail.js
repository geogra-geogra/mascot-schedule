function getEvents(data, filterCallback) {
    return data.filter((item) => {
        ...
        const aDate = new Date(a.date.split('/').join('-'));
        const bDate = new Date(b.date.split('/').join('-'));
...
function createEventDetails(item, mascots) {
    ...
    if (item.title.includes("[ファーム]")) {
        title = `<span class="farm">${item.title}</span>`;
    }
    ...
    characters.forEach(character => {
        ...
        characterHTML += mascotMatch ? `<a href="https://baseball-mascot.com/profile/${mascotMatch.name}.html">${character}</a>/` : `${character}/`;

        if (!item.title.includes("@") || (mascotMatch && mascotMatch.home !== item.spot)) {
            hasSpecialEvent = true; // Set the flag if conditions are met
            titleClass = 'red'; // If title does not include '@' or the mascot's home doesn't match the item's spot, set class to 'red'
        }
    });
...
export function updateScheduleCommon(scheduleDivId, filterCallback) {
    ...
    fetch('../data/mascot.json?20231122')
        ...
        .then(mascots => {
            fetch(`../data/schedule.json?20231122`)
                .then(response => {
                    if (!response.ok) {
                        throw new Error("HTTP error " + response.status);
                    }
